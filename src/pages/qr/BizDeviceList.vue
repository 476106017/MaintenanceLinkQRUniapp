...
import {columns} from './BizDeviceData';
defineOptions({
  name: 'BizDeviceList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/qr/bizDevice/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/qr/bizDevice/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
